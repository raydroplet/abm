    ...
    pub fn run(self) -> eframe::Result<()> {
        ...
        let options = eframe::NativeOptions {
            viewport: egui::ViewportBuilder::default()
                .with_inner_size([self.viewport_size.x as f32, self.viewport_size.y as f32])
...
impl eframe::App for ViewEGUI {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if new_size != self.viewport_size {
            ...
            let _ = self
                .command_sender
                .send(EngineCommand::UpdateViewport(new_size).into());

            println!("Viewport resized to: {:?}", new_size); // Debug confirmation
        }
